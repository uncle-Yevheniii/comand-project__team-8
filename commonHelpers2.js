import{c as s}from"./assets/sptite-51f64df6.js";import{a as m,S as x}from"./assets/vendor-3d334923.js";async function P(){const t="https://food-boutique.b.goit.study/api/products/categories";try{return(await m.get(t)).data}catch(e){return console.error("Помилка під час запиту:",e),[]}}async function b({keyword:t,category:e,page:o,limit:r}){const c="https://food-boutique.b.goit.study/api/products";return(await m.get(c,{params:{keyword:t,category:e,page:o,limit:r}})).data}async function I(){const t="https://food-boutique.b.goit.study/api/products/popular";return(await m.get(t)).data}const M=document.querySelector(".filters-form"),i=document.querySelector(".filters-form-input"),n=document.querySelector(".filters-form-select-category"),j=t=>t.map(e=>`<option value="${e.trim()}">${e.replace("_"," ").replace("_"," ")}</option>`).join("");let S=[];const O=async()=>{S=[...await P()];const e=j(S);n.insertAdjacentHTML("beforeend",e),new x({select:"#category",settings:{showSearch:!1}})};O();const g=async t=>{const e=await b(t);console.log(e.results)},J=t=>{t.preventDefault();const e=i.value.trim(),o=n.value,r={keyword:e||null,category:o==="Show all"?null:o.trim()||null};localStorage.setItem("LS_QUERY_KEY",JSON.stringify(r)),g(r)},N=t=>{if(t.target.value===""){const e=n.value,o={keyword:null,category:e==="Show all"?null:e.trim()||null};localStorage.setItem("LS_QUERY_KEY",JSON.stringify(o)),g(o)}},T=t=>{const e=i.value.trim(),o=t.target.value,r={keyword:e||null,category:o.trim()||null};localStorage.setItem("LS_QUERY_KEY",JSON.stringify(r)),g(r)};window.addEventListener("load",()=>{const t=JSON.parse(localStorage.getItem("LS_QUERY_KEY"))||{keyword:null,category:null};i.value=t.keyword,n.value=t.category,g(t),M.addEventListener("submit",J),n.addEventListener("change",T),i.addEventListener("input",N)});const k=document.querySelector(".products-list"),A={keyword:null,category:null,page:1,limit:6};async function H(){return localStorage.getItem("settings")?await w(JSON.parse(localStorage.getItem("settings"))):await w(A)}async function w(t){try{const e=await b(t);k.innerHTML=Y(e.results)}catch(e){console.log(e.message)}}function Y(t){return t.map(({name:e,img:o,category:r,size:c,popularity:a,price:y,is10PercentOff:v,_id:q})=>(r.includes("_")&&(r=r.split("_").join(" ")),`
